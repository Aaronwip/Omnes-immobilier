<?php
    ...
    $client_id = isset($_SESSION['id_user']) ? intval($_SESSION['id_user']) : null;
    ...
    // Envoi de message
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['envoyer_message'], $_POST['msg'], $_POST['agent_id_hidden'])) {
        if (!$client_id) {
            echo "<script>alert('Connectez-vous pour envoyer un message'); window.location.href='login.php';</script>";
            exit;
        }

        $contenu = $mysqli->real_escape_string($_POST['msg']);
        $agent_id_msg = intval($_POST['agent_id_hidden']);

        $mysqli->query("INSERT INTO messages (client_id, agent_id, contenu) VALUES ($client_id, $agent_id_msg, '$contenu')");
    }
...
<head>
    ...
    <style>
        ...
        .form-container .btn {
            width: 80%; /* même largeur que textarea */
            padding: 12px;
            font-size: 16px;
        ...
        ?>
            ...
            <div class="form-popup" id="<?= $chatId ?>">
                <form method="post" class="form-container">
                    ...
                    <input type="hidden" name="agent_id_hidden" value="<?= $rdv['id_agent'] ?>">
                    <button type="submit" class="btn" name="envoyer_message">Envoyer</button>
