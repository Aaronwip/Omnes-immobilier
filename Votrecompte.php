...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $role = $mysqli->real_escape_string($_POST["critere"] ?? 'client');
    ...
    if ($_POST["action"] == "inscription") {
        ...
        if ($mysqli->query($sql)) {
            ...
            if ($role === 'agent') {
                $sqlAgent = "INSERT INTO agents (nom, prenom, email, telephone, photo, specialite_id)
                             VALUES ('$nom', '$prenom', '$email', '', '', NULL)";
                $mysqli->query($sqlAgent); 
            }
            
            if ($role === 'agent') {
                header("Location: inscription_agent.php");
            } else {
